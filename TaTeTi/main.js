	...
	$("#board tr td").click(function () {
		if ($(this).text() == "" && play) {
			...
			if (checkForWinner() != -1 && checkForWinner() != "") {
				if (checkForWinner() == "X") {
					$('body').append('<div class="winner"><span>Ganador</span>X</div><button onclick="location.reload();" id="reload">Volver a jugar</button>');
					$('.winner').css('background-color', '#61892f');
					$('#reload').css('color','#61892f');
				} else {
					$('body').append('<div class="winner"><span>Ganador</span>O</div><button onclick="location.reload();" id="reload">Volver a jugar</button>');
